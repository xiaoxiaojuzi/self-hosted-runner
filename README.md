...
*Brief: Do not use AWS IAM User Access Keys directly in GitHub Actions Self-Hosted Runner on EKS. Instead, define an IAM role that has minium permissions for Self-Hosted Runner and launch the Self-Hosted Runner with the role.*
...
Sometimes, we need to use AWS resources on GitHub Actions self-hosted runner. For example, after building a Docker image, we need to push the Docker image to AWS ECR. This requires that the self-hosted runner on EKS has permission to ECR. There are two ways to do it:
...
The access keys are long-term credential for an IAM user and consist of two parts: an access key ID `AWS_ACCESS_KEY_ID` and a secret access key `AWS_SECRET_ACCESS_KEY`.
...
There is another option to manage identity and access, it is temporary security credentials (IAM roles). Due to Amazon EKS being integrated with IAM deeply, you can securely control access to AWS resources using IAM role. Next, let's see how to use AWS Assume Role.
    ...
    steps:
      ...
      - name: Push image to Amazon ECR
        ...
        run: |
          ...
          docker push $ECR_REGISTRY/$ECR_REPOSITORY:$CACHE_IAMGE_TAG
```
For now, we don't need the step 2 `Configure AWS credentials` on Using AWS IAM User Access Keys. Also, we don't need to store the access keys on Github secrets.
We use AWS assume role to grant permission for the self-hosted runner on EKS to access ECR.
**Enable it in two steps:**  
* IAM roles for service accounts
* Configure the service account for runner
...
1. Associate an IAM role with a service account
We can associate an IAM role with a Kubernetes service account. This service account can then provide AWS permissions to the containers in any pod that uses that service account.
...
`Runner` and `RunnerDeployment` kinds on ARC respectively corresponds to the `Pod` and `Deployment` kinds on K8S. The following code takes `RunnerDeployment` for example.
...
```
## Feature work
In the above, We show how self-hosted runners on AWS ECK uses credential to access AWS resource.
If we use GitHub-hosted runners or self-hosted runners on Other cloud, how to use credential to access AWS resource. The only way is use User Access Key, or there is other way.
## Reference:
